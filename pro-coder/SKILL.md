...
| **What an image / video / audio / PDF / Office file is** (dimensions, duration, pages, extracted text, stored description) | `lens describe <file>` — read this **before** viewing or opening the file |
| **Record what you saw** after viewing an image (or reading a PDF/doc the extractor could not) so no model ever re-reads it | `lens describe <file> --text "<what it shows / contains>" --by claude` |
...
6. **Media and documents: view once, describe once, search forever.** Before opening an image, video, audio file, PDF or Office document, run `lens describe <file>`. If it already carries a description (yours, another model's, or the user's) or enough extracted text, use that and do not open the file. If it does not, view/read the file **once** (Claude can view images directly; PDFs and Office docs have their text extracted by lens; videos/audio get duration only — ask the user for content when it matters), then store what you learned with `lens describe <file> --text "…" --by claude`. Every later lookup, by any model in any session, is a `lens search` hit. Re-viewing a file that already has a description is a token-discipline violation unless the file's bytes changed (lens shows the new dimensions/size) or the task is specifically about its visual details.

**Any client, any project, any file.** The same verbs are exposed over MCP (`lens mcp`) with identical output, a per-call `root` argument for multi-project sessions, auto-bootstrap of a missing index, and the same freshness check — a Codex/GPT/Cursor agent driving lens gets exactly what Claude gets. Every file in the project is indexed: code in eight languages as a symbol graph, every text file (any language, docs, config, shell) as full text, and every binary (images, video, audio, PDF, Office, archives) as an asset with extracted metadata/text plus a stored description. The lens index also makes `.claude/state/` notes searchable, so lens doubles as the cross-session memory for whichever model is at the keyboard.
