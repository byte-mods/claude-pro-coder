...
| **Written-brief enforcement** | Before any task is implemented, pro-coder posts a visible `**Task brief (T<n>):**` block in the conversation covering the goal, files implicated, edge cases, failure modes, and verification approach. Before every super-qa spawn (task-level and section-level), it posts a visible `**Super-qa briefing:**` block listing exactly what the reviewer should verify. Super-qa itself posts a visible `**Super-qa test plan:**` block before its verdict. The user sees what the agent is about to do, before it does it. | `pro-coder/SKILL.md` (P4 step 0a, P4.5, P5) |
...
# 3. Smoke-test lens on a supported-language project (Rust, Python, TS/JS, Go, Dart, Java)
cd ~/some-project
lens init
lens index
...
**Doc comments come first.** Per-language extractors harvest `///` / `/** */` (Rust, Dart, Java), docstrings (Python), JSDoc (TS/JS), and `//` (Go) at index time. `lens follow` surfaces them as a `> blockquote` ahead of signature and body — for well-documented code Claude often doesn't need the body at all.
